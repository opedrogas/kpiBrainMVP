...
export const generatePerformancePDF = (
  ...
  try {
    ...
    const reportTitle = viewType === 'weekly' && weekData 
      ? `Week ${weekData.week}, ${weekData.year} Performance Report`
      : `${month} ${year} Performance Report`;
    ...
    // Note: reviewItems are already filtered by period in the calling function
    const kpisWithReviews = kpis.filter(kpi => {
      const kpiReviews = filteredReviews.filter(review => review.kpi === kpi.id);
    ...
    const filename = viewType === 'weekly' && weekData 
      ? `Week_${weekData.week}_${weekData.year}_Performance_Report.pdf`
      : `${month}_${year}_Performance_Report.pdf`;
